...
import { addBroadcastToStore } from "@/lib/admin/broadcast-store";

/**
 * Generates a clean, formatted 1-page Sunday PDF / Text Document
 */
function generateCondensedDigestText(
...
async function executeDispatch(params: {
  ...
  // 2. WEEKLY PERFORMANCE AUDIT & CONDENSED REVISION DIGEST
    ...
    documentContent = generateCondensedDigestText(
      summary,
      params.starredNotes,
      params.missedFlashcards
    );
  }
  ...
  const broadcastTitle =
    ...
    (params.type === "daily_brief"
      ? "📰 Daily GS Editorial Brief"
      : params.type === "weekly_audit"
      ? "📊 Sunday Performance Audit"
  ...
  return {
    message:
      token && targetChatId
        ? "Dispatched successfully to Telegram."
        : "Generated digest successfully (No bot token configured).",
